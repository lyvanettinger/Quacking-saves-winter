...
int main(int argc, char** argv)
{
	...
	gameStateManager->SetTarget(surface);
	gameStateManager->SetAssetManager(assetManager);
	timer t;
	t.reset();
	while (!exitapp)
	{
		...
		case 1:
			...
			{
				// set to game over
				gameStateManager->SetGameState(3);
				gameStateManager->WriteGameOver();
				// clean up
				game->Reset();
			}
			else
		...
		while (SDL_PollEvent( &event )) 
		{
			...
			case SDL_KEYDOWN:
				...
				case 1:
					...
					{
						gameStateManager->SetGameState(2);
					}
					break;
				// paused
				case 2:
